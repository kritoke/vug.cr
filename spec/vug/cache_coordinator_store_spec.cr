...
describe Vug::CacheCoordinator do
  it "store stores to memory cache and calls cache_manager.set" do
    ...
    begin
      ...
      coord.store("https://example.com/favicon.ico", path)
  ...
  it "store does not store relative paths in memory cache" do
    mem = Vug::MemoryCache.new
    test_cm = TestCMStore2.new(mem)

    coord = Vug::CacheCoordinator.new(Vug::Config.default, mem, test_cm)
    coord.store("https://example.com/favicon.ico", "relative/path.png")
